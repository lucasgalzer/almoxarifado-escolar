...
async function indicadores(req, res, next) {
  try {
    ...
    // IDs de categorias com controle individual
    const categoriasIndividuais = await db('categorias')
      .where({ instituicao_id, tipo_controle: 'individual' })
      .pluck('id')
    ...
    const [
      ...
      db('produtos')
        .where({ instituicao_id })
        .whereRaw('quantidade_atual <= quantidade_minima AND quantidade_atual > 0 AND quantidade_minima > 0')
        .where(q => {
          if (categoriasIndividuais.length > 0) {
            q.whereNotIn('categoria_id', categoriasIndividuais)
          }
        })
        .count('id as total').first(),

      db('produtos')
        .where({ instituicao_id, quantidade_atual: 0 })
        .where(q => {
          if (categoriasIndividuais.length > 0) {
            q.whereNotIn('categoria_id', categoriasIndividuais)
          }
        })
        .whereRaw('quantidade_minima > 0')
        .count('id as total').first(),

      db('emprestimos as e')
        .join('produtos as p', 'e.produto_id', 'p.id')
        .where({ 'p.instituicao_id': instituicao_id, 'e.status': 'emprestado' })
        .count('e.id as total').first(),

      db('manutencoes as m')
        .join('produtos as p', 'm.produto_id', 'p.id')
        .where({ 'p.instituicao_id': instituicao_id })
        .whereIn('m.status', ['aguardando', 'em_conserto'])
        .count('m.id as total').first(),

      db('solicitacoes')
        .where({ instituicao_id, status: 'pendente' })
        .count('id as total').first(),
    ])

    const produtosBaixo = await db('produtos')
      .where({ instituicao_id })
      .whereRaw('quantidade_atual <= quantidade_minima AND quantidade_minima > 0')
      .where(q => {
        if (categoriasIndividuais.length > 0) {
          q.whereNotIn('categoria_id', categoriasIndividuais)
        }
